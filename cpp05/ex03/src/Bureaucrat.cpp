...
void Bureaucrat::signForm( AForm & F )
{
	...
	if (F.getSigned() == true)
	{
		std::cout	<< "Bureaucrat " << this->getName() << " couldn't sign form "
					<< this->getName() << ", because it has already been signed." << std::endl;
		return ;
	}
...
void Bureaucrat::executeForm(AForm const & form)
{
	if (form.getSigned() == false)
		std::cout << this->getName() << " can't execute " << form.getName() << ", because the form isn't signed." << std::endl;
	else if (this->getGrade() > form.getExecrequirement())
